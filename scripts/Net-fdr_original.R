...
getwd()
png("Number_of_edges_0309.png")
pdf("Number_of_edges_0309.pdf")
...
trueValues<-matrix(nrow=length(seq(0.3, 0.9 , .02)), data=tp, byrow = T)
trueMEAN<-rowMeans(trueValues)

rr<-unlist(getPositiveValues(RandomAdjMatrix))/2
randomValues<-matrix(nrow=length(seq(0.3, 0.9 , .02)), data=rr, byrow = T)
