...
TPList<-mclapply(TrueList, CRobCor, mc.cores = 6)# paraleliza OK
length(TPList)
RandomList<-createRandomMatrix(TrueM = t_Zvals,  NumRandomM)
RList <- vector("list", length(RandomList))
RList<-mclapply(RandomList, CRobCor, mc.cores = 6)# paraleliza OK
TPList<-rep(TPList, NumRandomM)
##################################################
#ya tengo las 2 listas
#paso a extraer los valores
#1.- Number of nodes
RandomEdgesList<-getEdgesByRHO(RList, start=0.3, end=0.7, interval=0.02, cores=6);length(RandomEdgesList)#21
TrueEdgesList<-getEdgesByRHO(TPList, start=0.3, end=0.7, interval=0.02, cores=6); length(TrueEdgesList)#21#OK
mclapply(RandomEdgesList, mean, mc.cores=6)
mclapply(TrueEdgesList, mean, mc.cores=6)
#PLOTS
eje1<-seq(0.3,0.7,0.02)
trueEdges<-unlist(lapply(TrueEdgesList, mean))
randomEdges<- unlist(lapply(RandomEdgesList, mean))

png("Number_of_edges.png")
plot(eje1,trueEdges, main="Number of edges real / random data vs rho" ,
     type="l", xlab="number of edges", ylab="rho", col="blue")
lines(eje1, randomEdges, col="red")
legend(.4,2000 , c("real data","random data"), lty = 1, col = c("blue","red"))
dev.off()
##################################################################
png("ratio_number_of_edges.png")
...
TrueAdjMatrix<-getADJRHO(TPList, start=0.3, end=0.7 , interval=0.02, cores=6); length(TrueAdjMatrix)#10
RandomAdjMatrix<-getADJRHO(RList, start=0.3, end=0.7 , interval=0.02, cores=6); length(RandomAdjMatrix)#10
##################################################
#from here divide by 2
tp<-unlist(getPositiveValues(TrueAdjMatrix, cores = 6))/2
trueValues<-matrix(nrow=length(seq(0.3, 0.7 , .02)), data=tp, byrow = T)
trueMEAN<-rowMeans(trueValues)

rr<-unlist(getPositiveValues(RandomAdjMatrix, cores = 6))/2
randomValues<-matrix(nrow=length(seq(0.3, 0.7 , .02)), data=rr, byrow = T)
randomMEAN<-rowMeans(randomValues)
#number of positives
png("real_data_positiveValues.png")
...
legend(.4,2000 , c("edges","positives values"), lty = 1, col = c("red","blue"))
dev.off()
##################################################
png("random_data_positiveValues.png")
plot(eje1,randomMEAN, type="l", col="blue", main="Adj positive values and edges in random data")
lines(eje1,randomEdges, col="red")
legend(.4,1500 , c("edges","positives values"), lty = 1, col = c("red","blue"))
dev.off()
##################################################
#alltogether
png("numberOfEdges_positiveValues.png")
...
legend(.4,2000 , c("positives values real data",
                   "edges real values",
...
plot(eje1,  ratioNumberOfEdges, type="l", main="Ratios random/real", col="red", xlab="rho", ylab="ratio")
lines(eje1, ratioPositiveValues , col="blue", type="l") 
legend(.4,70 , c("ratio number of edges",
