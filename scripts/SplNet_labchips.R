...
?scale
#este es el que hay que editar con los nombres
...
commu[is.na(commu)]<-0
row.names(topo)
write.table(data.frame(topo, names, commu), file="allInfoLabChip048.tab", sep="\t")
########################################
##############
write_graph(g, format="edgelist", file="ejemplo048")